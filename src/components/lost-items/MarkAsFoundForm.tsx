// // src/components/lost-items/MarkAsFoundForm.tsx
...
// export const MarkAsFoundForm: React.FC<MarkAsFoundFormProps> = ({
//   onSubmit,
//   isLoading = false,
//   onCancel,
// }) => {
...
//           Marking this item as found will move it to the Found Items list.
...
interface MarkAsFoundFormProps {
  ...
  isLoading?: boolean;
  onCancel?: () => void;
  existingImageUrl?: string | null; // ✅ NEW: pre-fill from lost item
}
...
export const MarkAsFoundForm: React.FC<MarkAsFoundFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ IMAGE IS REQUIRED (pre-filled or uploaded)
    if (!imageUrl) {
      showToast('Please upload an image of the found item', 'error');
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <ImageUpload
        onFileSelect={handleImageUpload}
        isUploading={isUploading}
        previewUrl={imageUrl ?? undefined}   // null → undefined to satisfy prop type
        onRemove={() => setImageUrl(null)}   // Allow clearing
        label="Upload Found Item Image"
        // Note: required prop not needed because we validate in handleSubmit
      />
      ...
      <div className="flex gap-2">
        <Button
          ...
          variant="glass-green"
          isLoading={isLoading || isUploading}
          disabled={!turnInBy.trim() || !foundBy || !imageUrl} // image required
          className="flex-1"
        >
          Confirm Found
        </Button>
...
export default MarkAsFoundForm;