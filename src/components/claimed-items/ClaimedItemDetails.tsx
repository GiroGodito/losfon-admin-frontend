...
//     return <span className={getBadgeClasses('Resolved')}>Resolved</span>;
//   };
...
//                 {getStatusBadge(true)}
//                 {item.userId && (
//                   <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border backdrop-blur-sm text-white bg-blue-600/80 border-blue-400/60 shadow-lg shadow-blue-500/30">
...
interface ClaimedItemDetailsProps {
  item: ClaimedItem;
  onUndoMarkAsClaimed?: (id: number) => void;   // ✅ Undo goes back to Found Item
  onDelete?: (id: number) => void;
  showActions?: boolean;
}
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-sm rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-6">
        ...
        {/* Title */}
        <div className="mb-4">
          <h3 className="text-xl font-bold text-white break-words">{item.itemDescription}</h3>
        </div>
        ...
        {/* System Metadata */}
        <div className="flex items-center justify-center gap-4 text-xs text-gray-500 bg-gray-800/20 rounded-lg border border-gray-700/50 px-4 py-2.5 flex-wrap">
          <span>Created <span className="text-gray-400">{formatRelativeTime(item.createdAt)}</span></span>
          <span className="text-gray-700">•</span>
          <span>Updated <span className="text-gray-400">{formatRelativeTime(item.updatedAt)}</span></span>
        </div>
