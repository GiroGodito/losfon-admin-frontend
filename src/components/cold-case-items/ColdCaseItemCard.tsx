// src/components/cold-case-items/ColdCaseItemCard.tsx
import React from 'react';
import type { ColdCaseItem } from '../../types/cold-case-item.types';
import { Button } from '../common/Button';
import { useNavigate } from 'react-router-dom';
import { formatDate } from '../../lib/date';
import { CalendarIcon, UserIcon, PhoneIcon, ClockIcon } from '@heroicons/react/24/outline';

interface ColdCaseItemCardProps {
  item: ColdCaseItem;
  rowNumber?: number;
...
export const ColdCaseItemCard: React.FC<ColdCaseItemCardProps> = ({
  item,
  rowNumber,
  onMarkAsFound,
  onDelete,
      ...
      <div className="relative w-full h-44 rounded-t-xl overflow-hidden bg-gray-800/50 flex-shrink-0">
        {item.filePath ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-800/80 to-gray-900/80">
            <ClockIcon className="w-12 h-12 text-gray-600 mb-2" />
      ...
      <div className="p-4 flex flex-col flex-1 relative">
        ...
        <div className="space-y-2 flex-1">
          ...
          <div className="flex items-center justify-center gap-2 text-sm text-gray-400">
            ...
            <span className="text-xs truncate break-words whitespace-normal">{item.reportedBy}</span>
          </div>
          
          <div className="flex items-center justify-center gap-2 text-sm text-gray-400">
            <PhoneIcon className="w-4 h-4 text-gray-500 flex-shrink-0" />
            <span className="text-xs">{item.contactNumber}</span>
          </div>
        ...
        {showActions && (
          <div className="mt-4 pt-3 border-t border-gray-800/60 flex flex-wrap gap-2">
            <Button
              variant="glass-grey"
              size="sm"
              onClick={() => navigate(`/cold-case/${item.id}`)}
