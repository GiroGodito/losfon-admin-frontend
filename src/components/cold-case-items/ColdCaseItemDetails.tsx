// src/components/cold-case-items/ColdCaseItemDetails.tsx
...
interface ColdCaseItemDetailsProps {
  item: ColdCaseItem;
  onMarkAsFound?: (id: number) => void;
  onDelete?: (id: number) => void;
  showActions?: boolean;
}

export const ColdCaseItemDetails: React.FC<ColdCaseItemDetailsProps> = ({
  item,
  onMarkAsFound,
  onDelete,
  showActions = true,
   ...
   * Returns glass-yellow or solid yellow variant based on image presence
   */
  const getStatusBadge = (hasImage: boolean): React.ReactNode => {
    ...
    return <span className={getBadgeClasses('Cold Case')}>Cold Case</span>;
  };
    ...
    return (
      <div className="mt-6 pt-6 border-t border-gray-800">
        <div className="flex flex-wrap gap-3">
          {onMarkAsFound && (
            <Button variant="glass-green" onClick={() => onMarkAsFound(item.id)}>
  ...
  const renderStatusTimeline = (): React.ReactNode => {
    const events = [
      ...
      ...(item.dateTransferred
        ...
        : []),
      {
        label: 'Days in Cold Case',
        value: `${item.daysInColdCase} days`,
        icon: <ClockIcon className="w-4 h-4" />,
      },
    ];
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Button
            ...
            aria-label="Return to cold case items list"
            className="!px-3"
          >
            <ArrowLeftIcon className="w-4 h-4" />
          </Button>
          <h2 className="text-lg font-semibold text-white">Cold Case Item Details</h2>
        </div>
        ...
        <div className="flex items-center justify-center gap-4 text-xs text-gray-500 bg-gray-800/20 rounded-lg border border-gray-700/50 px-4 py-2.5 flex-wrap">
          <span>
            Created <span className="text-gray-400">{formatRelativeTime(item.dateReported)}</span>
          </span>
...
export default ColdCaseItemDetails;