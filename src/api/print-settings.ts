...
// ✅ Used for most endpoints (LostItems, FoundItems, ClaimedItems, etc.)
export interface PaginatedResponse<T> {
  items: T[];
  totalCount: number;
  page: number;
...
// ✅ ACTUAL backend response for /PrintSettings/logs
export interface PrintLogsApiResponse {
  success: boolean;
  data: PrintLog[];
  pagination: {
    page: number;
...
export const printSettingsApi = {
  ...
  // ✅ FIXED: Uses PrintLogsApiResponse, not PaginatedResponse
  getPrintLogs: (params: PrintLogsQueryParams = {}): Promise<PrintLogsApiResponse> => {
    const queryParams = new URLSearchParams();
    if (params.reportType) queryParams.append('reportType', params.reportType);
    if (params.dateFrom) queryParams.append('dateFrom', params.dateFrom);
    if (params.dateTo) queryParams.append('dateTo', params.dateTo);
    if (params.page) queryParams.append('page', String(params.page));
    if (params.pageSize) queryParams.append('pageSize', String(params.pageSize));
    return api.get<PrintLogsApiResponse>(`/PrintSettings/logs?${queryParams.toString()}`);
  },
