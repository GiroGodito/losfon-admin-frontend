...
// THIS IS THE LATEST IMPLEMENTATION OF THE CODE WITH ERROR ON RESPONSE TYPE
// export interface PrintRequest {
//   dateFrom?: string;
//   dateTo?: string;
//   searchTerm?: string;
...
/**
 * PDF API - Uses the api client which proxies through Vercel
 * The responseType: 'blob' tells the client to handle blob responses
 */
export const pdfApi = {
  generateLostItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/lost-items', data, { 
      responseType: 'blob' 
    }),

  generateFoundItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/found-items', data, { 
      responseType: 'blob' 
    }),

  generateClaimedItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/claimed-items', data, { 
      responseType: 'blob' 
    }),

  generateDisposalItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/disposal-items', data, { 
      responseType: 'blob' 
    }),

  generateColdCaseItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/cold-case-items', data, { 
      responseType: 'blob' 
    }),

  generateDonatedItems: (data: PrintRequest): Promise<Blob> =>
    api.post<Blob>('/Pdf/donated-items', data, { 
      responseType: 'blob' 
    }),
};